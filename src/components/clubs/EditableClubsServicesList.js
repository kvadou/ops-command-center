...
const CLUB_LABELS = ['Club - Park Slope', 'Club - Park Slope Support'];

export default function EditableClubsServicesList({ selectedClub = 'all' }) {
  ...
  const fetchServices = async () => {
    try {
      ...
      // Build label filter based on selected club
      let labelFilter = '';
      if (selectedClub !== 'all') {
        const clubLabelMap = {
          'park-slope': 'Club - Park Slope',
        };
        labelFilter = clubLabelMap[selectedClub] || '';
      }
                  ...
                  {termBillingEnabled && (
                    <>
                      <TextField
                        ...
                        fullWidth
                        placeholder={price ? String(price) : "25.00"}
                        InputProps={{
