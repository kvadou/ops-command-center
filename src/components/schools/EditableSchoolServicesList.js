...
const SCHOOL_LABELS = ['School - NYC', 'School - LA', 'School - SF', 'School - Hamptons'];

export default function EditableSchoolServicesList({ selectedSchool = 'all' }) {
  ...
  const fetchServices = async () => {
    try {
      ...
      // Build label filter based on selected school
      let labelFilter = '';
      if (selectedSchool !== 'all') {
        const schoolLabelMap = {
          ...
          'la': 'School - LA',
          'sf': 'School - SF',
          'hamptons': 'School - Hamptons',
        };
        labelFilter = schoolLabelMap[selectedSchool] || '';
      }
      
      let apiUrl = '/api/services';
      if (labelFilter) {
        apiUrl += `?label=${encodeURIComponent(labelFilter)}`;
      }
                  ...
                  <tr key={service.serviceId} className="hover:bg-neutral-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        {service.staffDiscountEnabled && (
                          <Tooltip title={`Staff Booking Form (${service.staffDiscountPercentMonthly || 20}% discount)`}>
                            <a
                              href={`/booking-forms/frontend?serviceId=${service.serviceId}&staff=true`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="p-2 text-green-600 hover:text-green-700 hover:bg-green-50 rounded-lg transition-colors"
                              title="Staff Booking Form"
                            >
